  ...
  class FieldSettingsController < ApplicationController
    ...
    def create
      ...
      if @field_setting.save
        redirect_to @field_setting.merge(:only_path => true), notice: 'Field setting was successfully created.'
      else
        render :new
      end
    end
    ...
    def update
      if @field_setting.update(field_setting_params)
        redirect_to @field_setting.merge(:only_path => true), notice: 'Field setting was successfully updated.'
      else
        render :edit
      end
    end
